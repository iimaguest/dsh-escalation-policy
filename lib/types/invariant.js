...
import { ESCALATION_POLICIES, isEscalationPolicy } from './index.js';
const PACKAGE_NAME = '@deepseek-ai/dsh-escalation-policy';
const ESCALATION_OUTCOMES = ['allowed-once', 'rejected', 'cancelled', 'unavailable'];
...
function validateEscalationEvent(trace, event, fail) {
    if (event.type === 'escalation/asked') {
        if (trace.openTurn === null)
            fail('escalation/asked appended outside any open turn');
        if (event.data.toolName.length === 0)
            fail('escalation/asked toolName must be non-empty');
        if (event.data.target.length === 0)
            fail('escalation/asked target must be non-empty');
        if (trace.pending.has(event.data.id))
            fail(`escalation/asked repeated open id ${JSON.stringify(event.data.id)}`);
        return { kind: 'asked', id: event.data.id };
    }
    if (event.type === 'escalation/decided') {
        if (trace.openTurn === null)
            fail('escalation/decided appended outside any open turn');
        if (!trace.pending.has(event.data.id))
            fail(`escalation/decided has no matching escalation/asked for id ${JSON.stringify(event.data.id)}`);
...
function applyEscalationTransition(pending, transition) {
...
const install = Object.assign((ctx, fail) => {
    ...
    const seed = (session) => {
        ...
        for (const event of session.snapshotEvents()) {
            ...
            if (transition !== undefined)
                applyEscalationTransition(trace.pending, transition);
        }
        return trace;
    };
    ...
    ctx.on('session/event', (session, event) => {
        ...
        if (event.type !== 'escalation/asked' && event.type !== 'escalation/decided')
            return;
        const candidate = staged.get(event);
        /* v8 ignore next -- internal/dispatch stages every package-owned pair event */
        if (candidate === undefined || candidate.session !== session)
            return fail('escalation audit event published without pre-commit validation');
        staged.delete(event);
